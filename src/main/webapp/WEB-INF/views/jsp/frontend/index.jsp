...
<div class="container">
    ...
    <div class="row">
        ...
        <c:if test="${!empty listTires}">
            <c:forEach items="${listTires}" var="product">
                <div class="col-md-3">
                    <p><c:out value="${product.getManufactorer().getTitle()}"/></p>
                    <p><c:out value="${product.getNomencloture()}"/></p>
                    <p><c:out value="${product.getTireWidth()}"/></p>
                    <p><c:out value="${product.getTireHeightProfile()}"/></p>
                    <p><c:out value="${product.getTireDiameterRim()}"/></p>
    ...
    <div class="row">
        <h2>Диски</h2>
        <c:if test="${!empty listDrives}">
            <c:forEach items="${listDrives}" var="product">
                <div class="col-md-3">
                    <p><c:out value="${product.getManufactorer().getTitle()}"/></p>
                    <p><c:out value="${product.getNomencloture()}"/></p>
                    <p><c:out value="${product.getDriveFirstPart()}"/></p>
                    <p><c:out value="${product.getDriveSecondPart()}"/></p>
                    <p><c:out value="${product.getDriveOuter()}"/></p>
                    <p><c:out value="${product.getDriveCenterPummetDiameter()}"/></p>
                </div>
            </c:forEach>
        </c:if>
    </div>
    ...
    <!-- прайс xls-->
    <div class="row">
        <p><a href="/static/price/price.xls">Скачать</a> прайс в формате xls</p>
    </div>
    <hr/>
